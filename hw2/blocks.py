...
class Linear(Block):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.normal(torch.zeros(self.out_features, self.in_features), wstd)
        self.b = torch.ones(self.out_features)
        # ========================
...
class Sigmoid(Block):
    ...
    def backward(self, dout):
        ...
        s = self.grad_cache['sigmoid']
        ds = torch.mul(s, 1 - s) # torch.mul is an elementwise multiplication
        dx = torch.mul(dout, ds) # torch.mul is an elementwise multiplication
        # ========================
...
class CrossEntropyLoss(Block):
    ...
    def forward(self, x, y):
        ...
        loss = torch.sum(-x_y + torch.logsumexp(x, dim=1))/N
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        dx = torch.exp(x)/torch.sum(torch.exp(x),dim = 1).reshape(-1,1) #softmax
        ones = torch.zeros_like(x)
        ones[range(ones.shape[0]),y] = 1
        dx = dout * (dx - ones)/N
        # ========================
...
class Sequential(Block):
    ...
    def backward(self, dout):
        ...
        for i in range(nblocks):
            din = self.blocks[nblocks-i-1].backward(din)
        # ========================
    ...
    def params(self):
        ...
        paramss = []
        for block in self.blocks:
            paramss.append(tuple(block.params()))
        # ========================
...
class MLP(Block):
    ...
    def __init__(self, in_features, num_classes, hidden_features=(),
        ...
        # ====== YOUR CODE: ======
        h = len(hidden_features) # number of hidden layers
        ...
        blocks.append(activation_func)
        if dropout:
            blocks.append(Dropout(p=dropout))

        for i in range(1, h):
            blocks.append(Linear(hidden_features[i-1], hidden_features[i]))
            blocks.append(activation_func)
            if dropout:
                blocks.append(Dropout(p=dropout))
